	...
	if(sel) {
		...
		if((widget->type == widget_editable) && (extractSelectablePartFromWidget(widget)->state == widget_edit)) {
			...
			case field_uinteger16:
				...
				break;
			case field_uinteger32:
				ui32 = *(uint32_t*)widget->editable.inputData.getData();
				ui32 = ui32 + inc;
				if(ui32 > widget->editable.max_value)
					ui32 = widget->editable.max_value;
				else if(ui32 < widget->editable.min_value)
					ui32 = widget->editable.min_value;
				widget->editable.setData(&ui32);
				break;
			case field_string:
